---
title: "A Snapshot of the coronavirus outbreak in Germany in April 2020"
author: "Nancy Yu"
...
*Note: added Saarland data on April 11 as its cases and deaths are rising quite fast while the numbers in many other states are stabilizing.*
...
```{r cum_linear, echo = F, message = F, fig.width=8, fig.height=5, dpi = 96}
...
cum.plot <- ggplot(data=states.final, aes(x=Days_100, y=Total_Cases, group=State, color=State)) +
            geom_line() + geom_point(size = 0.3) + scale_color_brewer(palette = 'Paired') +
            scale_x_continuous('Days since the 100th case', limits=c(0,35)) + scale_y_continuous("Total cases") + 
...
```{r cum_logscale, echo = F, message=F, fig.width=8, fig.height=5, dpi=96}
...
cum.plot2 <- ggplot(data=states.final, aes(x=Days_100, y=Total_Cases, group=State, color=State)) +
  geom_line() + geom_point(size=0.3) + scale_color_brewer(palette = 'Paired') +
  scale_x_continuous(name='Days since the 100th case', limits=c(0,35)) + 
...
```{r cum_per100, echo = F, message = F, fig.width=8, fig.height=5, dpi = 96}

# Plot 1 == Linear scale for total number of cases
per100.plot <- ggplot(data=states.final, aes(x=Days_100, y=Per100, group=State, color=State)) +
            geom_line() + geom_point(size = 0.3) + scale_color_brewer(palette = 'Paired') +
            scale_x_continuous('Days since 100 cases', limits=c(0,35)) + scale_y_continuous("Cumulative cases per 100,000") + 
            #ggtitle('Linear scale') +
            theme_minimal()
...
This plot shows the increase in the number of confirmed new cases. Each data point is averaged over 5 previous consecutive days. Note: This is not necessarily reflective in the increase of *actual* number of cases each day, since the number of tests for the virus has been increasing as the pandemic progressed.


```{r new_cases5days, echo = F, message = F, fig.width=8, fig.height=5, dpi=96}
#new.cases <- read_csv(here("static", "data", "GermanyNewCases10States.csv"))

new5days <- states.final[states.final$Avg_5days > 0,]
new5days$Dates <- as.Date(new5days$Dates)

newcase5.plot <- ggplot(data=new5days, aes(x=Dates, y=Avg_5days, group=State, color=State)) +
            geom_line() + geom_point(size = 0.3) + scale_color_brewer(palette = 'Paired') +
            scale_x_date(name='Date', labels=date_format('%Y-%m-%d')) + 
            scale_y_continuous(name="Number of new cases (averaged over 5 days)") + 
            #ggtitle('Linear scale') +
...
_*April 9, 2020*: The graph doesn't show it (5 day average), but the numbers seem to be going up again. Hard to say what the trend is until after Easter holidays. Are the tests processed slower during the holidays? We will see next week._ 
...
```{r cum_deaths, echo = F, message = F, fig.width=8, fig.height=5, dpi=96}
...
deaths.plot <- ggplot(data=deaths.final, aes(x=DeathsDays_5, y=Deaths, group=State, color=State)) +
   geom_line() + geom_point(size = 0.3) + scale_color_brewer(palette = 'Paired') +
   scale_x_continuous('Days since the 5th death', limits=c(0,35)) + 
...
##### Number of confirmed new deaths since March 20,2020

This plot shows the increase in the number of confirmed deaths. Each data point is averaged over past 5 consecutive days. 

```{r deaths_5days, echo = F, message = F, fig.width=8, fig.height=5, dpi=96}
#new.cases <- read_csv(here("static", "data", "GermanyNewCases10States.csv"))

states.final$Dates <- as.Date(states.final$Dates)
death5days <- states.final
death5days <- death5days[death5days$Dates >= as.Date('2020-03-20'),]

death5days.plot <- ggplot(data=death5days, aes(x=Dates, y=Death_5days, group=State, color=State)) +
            geom_line() + geom_point(size = 0.3) + scale_color_brewer(palette = 'Paired') +
            scale_x_date(name='Date', labels=date_format('%Y-%m-%d')) + 
            scale_y_continuous(name="Number of new deaths (averaged over 5 days)") + 
            #ggtitle('Linear scale') +
            theme_minimal() + theme(axis.text.x = element_text(angle=20, hjust=0.5, vjust=0.5))

print(death5days.plot)
...
_**April 16, 2020**: Sure enough, the rates of reported deaths went back up on work days. However, given that the rates of deaths lag behind the rates of confirmed new cases, I expect these numbers to go down for real next week._
