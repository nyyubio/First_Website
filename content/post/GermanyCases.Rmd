...
This is my attempt to visualize some SARS-CoV-2 data with some R code (please pardon the non-optimal layout). German data is obtained from the [Robert Koch Institute]. Data used from March 15-April 4, 2020 (not updated every day).
...
cum.plot <- ggplot(data=states.final, aes(x=Days_100, y=Total_Cases, group=State, color=State)) +
            ...
            scale_x_continuous('Days since the 100th case', limits=c(0,25)) + scale_y_continuous("Total cases") + 
...
cum.plot2 <- ggplot(data=states.final, aes(x=Days_100, y=Total_Cases, group=State, color=State)) +
  geom_line() + geom_point(size=0.3) + scale_color_brewer(palette = 'Paired') +
  scale_x_continuous(name='Days since the 100th case', limits=c(0,25)) + 
...
This graph shows the cumulative increase of cases adjusted for the population size of each state.  Per capita, Hamburg's cases are comparable with Bavaria and Baden-Württemburg (up until April 2, 2020).
...
per100.plot <- ggplot(data=states.final, aes(x=Days_100, y=Per100, group=State, color=State)) +
            geom_line() + geom_point(size = 0.3) + scale_color_brewer(palette = 'Paired') +
            scale_x_continuous('Days since 100 cases', limits=c(0,25)) + scale_y_continuous("Cumulative cases per 100,000") + 
            #ggtitle('Linear scale') +
            theme_minimal()
...
##### Number of confirmed new cases in Germany by States since March 1, 2020

This plot shows the increase in the number of confirmed new cases each day, each data point is averaged over 5 consecutive days. Note: This is not necessarily reflective in the increase of *actual* number of cases each day, since the number of tests for the virus has been increasing as the pandemic progressed.
...
newcase5.plot <- ggplot(data=new5days, aes(x=Dates, y=Avg_5days, group=State, color=State)) +
            ...
            #ggtitle('Linear scale') +
            theme_minimal() + theme(axis.text.x = element_text(angle=60, hjust=0.5, vjust=0.5))
...
##### Cumulative number of deaths in 10 German states with the most cases (in log scale)
...
deaths.plot <- ggplot(data=deaths.final, aes(x=DeathsDays_5, y=Deaths, group=State, color=State)) +
   geom_line() + geom_point(size = 0.3) + scale_color_brewer(palette = 'Paired') +
   scale_x_continuous('Days since the 5th death', limits=c(0,25)) + 
